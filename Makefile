...
sql-generate:
	...
	sqlboiler -d --wipe --add-enum-types --no-tests --add-soft-deletes -o internal/models/tplibrary -p tplibrary -c etc/config.yml psql
	go run internal/models/generate.go --step2
.PHONY: sql-generate

env-up:
	docker compose -p library -f docker-compose-dev.yaml up -d --wait
.PHONY: env-up

env-dn:
	docker compose -p library -f docker-compose-dev.yaml stop
	docker compose -p library -f docker-compose-dev.yaml rm -f
.PHONY: env-dn

prod-up:
	docker compose -p library -f docker-compose.yaml up -d --wait
.PHONY: prod-up

prod-dn:
	docker compose -p library -f docker-compose.yaml stop
	docker compose -p library -f docker-compose.yaml rm -f
.PHONY: prod-dn
